...
/**
 * <p>
 * Simple item reader implementation that opens a JDBC cursor and continually
 * retrieves the next row in the ResultSet.
 * </p>
 *
 * <p>
 * The statement used to open the cursor is created with the 'READ_ONLY' option
 * since a non read-only cursor may unnecessarily lock tables or rows. It is
 * also opened with 'TYPE_FORWARD_ONLY' option. By default the cursor will be
 * opened using a separate connection which means that it will not participate
 ...
 */
public class MonthTransDB<T> extends AbstractCursorItemReader<T> {
	...
	public MonthTransDB() {
		super();
		setName(ClassUtils.getShortName(MonthTransDB.class));
	}
	...
	protected void openCursor(Connection con) {
		try {
			long date1 = Long.parseLong(MonthTransService.tradeTime);
			// 20160600
			long dateBegin = date1 * 100;
			// 20160700
			long dateEnd = (date1 + 1) * 100;
			str=sql;
			str = str.replace("??", "<" + Long.toString(dateEnd));
			str = str.replace("?", Long.toString(dateBegin));
			if (isUseSharedExtendedConnection()) {
				preparedStatement = con.prepareStatement(str, ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY, ResultSet.HOLD_CURSORS_OVER_COMMIT);
			} else {
				preparedStatement = con.prepareStatement(str, ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY);
			}
