...
import com.yihuacomputer.fish.machine.service.base.DomainAtmModuleService;

@Service
@Transactional
public class AtmModuleService extends DomainAtmModuleService {
    @Autowired
    ...
    public IAtmModule get(String no) {
        IAtmModule atmModule = (IAtmModule) dao.getCriteria(AtmModule.class).add(Restrictions.eq("no", no)).uniqueResult();
        return atmModule;
    }
    ...
    public void remove(String no) {
        IAtmModule entity = get(no);
        dao.delete(entity.getId(), AtmModule.class);
    }
