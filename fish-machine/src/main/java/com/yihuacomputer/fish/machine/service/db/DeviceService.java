...
import com.yihuacomputer.fish.machine.service.base.DomainDeviceService;
...
@Service
@Transactional
public class DeviceService extends DomainDeviceService {
    @Autowired
    ...
    public IDevice get(String terminalId) {
        IDevice device = (IDevice) dao.getCriteria(Device.class).add(Restrictions.eq("terminalId", terminalId))
                .uniqueResult();
        return device;
    }
    ...
    public void remove(IDevice device){
		...
		if (personList == null || personList.isEmpty()) {
			...
			List<IQuittingNotice> list = (List<IQuittingNotice>) quittingNoticeService
					.list(filter);
			// 先删除运行参数
...
//		IDevice device = (IDevice) dao.getCriteria(Device.class).add(Restrictions.eq("ip", new IP(ip)))
//                .uniqueResult();
//		return device;
...
//      解决初始化ip地址重复报错的问题
	...
	public IDevice getByVirtual(String virtual) {
		  IDevice device = (IDevice) dao.getCriteria(Device.class).add(Restrictions.eq("virtual", virtual))
	                .uniqueResult();
	        return device;
	}
}
