...
public class LinkDeviceForm {
	...
	public LinkDeviceForm(IDevice device) {
		...
		// setCareLevel(device.getCareLevel() == null ? null :
		// String.valueOf(device.getCareLevel().getId()));
		// setCashType(device.getCashType() == null ? null :
		// String.valueOf(device.getCashType().getId()));
		...
		// setSp(device.getSp());
		/*
		 * if (device.getSerial() != null) { IDeviceExtend deviceExtend =
		 * device.getDeviceExtend();
		 * 
		 * setNetType(deviceExtend.getNetType() == null ? null :
		 * String.valueOf(deviceExtend.getNetType().getId()));
		 * 
		 ...
		 * 
		 * if (deviceExtend.getOpenTime() != null &&
		 * deviceExtend.getOpenTime().matches("\\d{2}:\\d{2}:\\d{2}")) {
		 * String[] openTimes = deviceExtend.getOpenTime().split(":");
		 * setOpenTimeHour(openTimes[0]); setOpenTimeMinute(openTimes[1]);
		 * setOpenTimeSecond(openTimes[2]); } if (deviceExtend.getCloseTime() !=
		 * null && deviceExtend.getCloseTime().matches("\\d{2}:\\d{2}:\\d{2}"))
		 * { String[] closeTimes = deviceExtend.getCloseTime().split(":");
		 * setCloseTimeHour(closeTimes[0]); setCloseTimeMinute(closeTimes[1]);
