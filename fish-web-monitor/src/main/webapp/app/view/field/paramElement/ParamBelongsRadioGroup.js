			...
			Ext.Ajax.request({
			    ...
			    success: function(response, opts) {
			        ...
			        for(var index=0;index<obj.length;index++){
			        	if(index==0){
			        	var radio = Ext.create("Ext.form.field.Radio",{boxLabel:obj[index].name, name:'appSystem',inputValue:obj[index].id,checked:true});
			        	_this.add(radio);
			        	}else{
			        		var radio = Ext.create("Ext.form.field.Radio",{boxLabel:obj[index].name, name:'appSystem',inputValue:obj[index].id});
				        	_this.add(radio);
			        	}
			        }
			    },
